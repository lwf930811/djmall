...
<script type="text/javascript">
    ...
    function search() {
        $.get(
            "<%=request.getContextPath()%>/auth/role/roleList",
            $("#frm").serialize(),
            function(data){
                var html = "";
                for (var i = 0; i < data.data.length; i++) {
                    ...
                    html += "<td><input type='button' value='编辑' onclick='updateRole("+role.roleId+")'>"
                    html += "<input type='button' value='删除' onclick='delRole("+role.roleId+")'>"
                    html += "<input type='button' value='关联资源' onclick='toRoleResource("+role.roleId+")'></td>"
                }
                $("#tbd").append(html);
    ...
    function updateRole(id){
        layer.open({
            ...
            area: ['380px', '70%'],
            //路径
            content:  '<%=request.getContextPath()%>/auth/role/toUpdateRole?id='+id
        });
    }
    ...
    function delRole(id){
        ...
        layer.confirm('确定删除吗?', {icon: 3, title:'提示'}, function(index) {
            $.post(
                "<%=request.getContextPath()%>/auth/role/delRole",
                {"id": id},
                function (data) {
                    if (data.code != -1) {
