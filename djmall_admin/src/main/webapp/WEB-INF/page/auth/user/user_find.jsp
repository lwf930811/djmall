...
<body>
    ...
    <form id = "fm">
       <input type="text" name="phone" id="userPhone" onblur="getSalt(this)" placeholder=" 手机号"><br/>
        图形验证码<br>
        <div class="verify-wrap" id="verify-wrap" style="float:left" ></div>
        <br><br><br>
        <input type = "text" name = "verify" id = "verify" placeholder=" 手机验证码"/>
        <input type="button"  value="获取验证码" onclick="getCode(this)" /><br/><br/>
        <input type="hidden" name="salt"  id="salt"/>
        <input type="password" name="password" id="pwd" placeholder="密码"/><br>
        <input type="password" name="userPwd1" id="pwd1" placeholder="确认密码"/><br>
    ...
    $.validator.setDefaults({
        submitHandler: function () {
            ...
            layer.msg('请稍等', {
                ...
                $("#pwd").val(newPassword);
                $.post("<%=request.getContextPath()%>/auth/user/findPhonePassword ",
                    $("#fm").serialize(),
                    function (data) {
                        layer.close(index);
                        if (data.code == -1) {
